...
export default function ProposalsPage() {
  ...
  const categoryData = useMemo(() => {
    ...
    const categoryConfig = {
      Treasury: { name: 'Treasury', color: '#C9A24D' },      // Aurelius gold
      Governance: { name: 'Governance', color: '#6366f1' }, // Indigo
      ...
      prevProposals.map((proposal) => {
        if (proposal.id === Number(proposalId)) {
          return {
            ...proposal,
            for_votes:
              vote === 'For' ? proposal.for_votes + 1 : proposal.for_votes,
            against_votes:
              vote === 'Against'
        ...
        <LayoutWrapper>
          ...
          <div className="mb-8 space-y-2">
            ...
            <p className="max-w-2xl text-base text-slate-600 dark:text-slate-400">
              Review, deliberate, and participate in protocol-level decisions
              shaping the future of Aurelius.
            </p>
          </div>
